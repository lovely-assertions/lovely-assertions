...
and its four calling forms are deliberately the same four as `pytest.approx`:
...
with no floor, write `rel=r, tol=0`.
...
These exist because `nan != nan`, which makes `assert value == float("nan")`
silently always false — a test that can never pass and never says so.

## Decimal and Fraction

They get `OrderedExpect`: everything above except the float-specific assertions
(`is_close_to`, `is_nan`, `is_infinite`), which do not apply.
...
`is_not_true` and `is_not_false` complete the four. They are not redundant with
each other: `is_not_true` asks "not the object `True`", which is a different
question from `is_false` only if you have narrowed the type loosely.
...
It passes whenever the antecedent is false, exactly as implication does. The
message names both sides, which `if a: assert b` cannot.
