...
It is the exhaustive list; the [guides](../README.md#guides) are the organised
tour.

## It is generated, and checked

The reference is not written by hand. Three things in it are derived from the
source on every run:
...
- **The dispatch table.** The order is the `@overload` chain on `expect()`, and
